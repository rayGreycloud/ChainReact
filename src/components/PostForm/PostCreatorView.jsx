...
class PostForm extends Component {
  ...
  submitForm = () => {
    ...
    postData.set(
      'statusChunks',
      JSON.stringify({ [this.state.statusPredicate]: this.state.statusSubject })
    )
        ...
        <Segment size="huge">
          <Form loading={this.state.loading}>
            <Segment size="big" className="form-container">
              ...
              >
                {this.state.filePreviewURL ? (
                  ...
                  <Icon
                    className="img-upload-placeholder-svg"
                    size="huge"
                    fitted
            ...
            <Segment size="big">
              ...
              <Form.Group inline>
                <Form.Dropdown
                  id="status-predicate-0"
                  options={statusModifiers}
                  type="dropdown"
                  label="What are you up to?"
                  onChange={(e, data) => {
                    this.setState({ statusPredicate: data.value })
                  }}
                ></Form.Dropdown>
                <Form.Input
                  id="status-subject-0"
