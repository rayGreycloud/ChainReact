...
export default class FullPostView extends Component {
  ...
  retrieveMeta(id) {
    return API.get(`/posts/content/${id}/meta`)
      .then(res => res.data)
  };
  ...
  playingPresent() {
    ...
    return playingChunk.length > 0 && (
      <div>
        <Header floated="left" color="grey">Playing: </Header>
  ...
  readingPresent() {
    ...
    return readingChunk.length > 0 && (
      <div>
        <Header floated="left" color="grey">Reading: </Header>
  ...
  statusPresent() {
    return this.state.postInfo.content && (
      <div>
  ...
  render() {
    return (
      <div className="full-post-body" style={{width: '90%', margin: 'auto', marginTop: '6em'}}>
        ...
        <Segment size="big">
            <Image centered={true} size="big" src={`${baseURL}/posts/content/${this.props.match.params.postID}`} />
            <Divider/>
